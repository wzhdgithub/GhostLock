...
for t in types:
    if t['kind'] == 4 and t['name'] == 'seccomp':
        print(f"=== seccomp tid={t['tid']} size={t['size']} vlen={t['vlen']}")
        for m in t['extra']['members']:
            print(f"    {m[0]:<32} off=0x{m[2]//8:x}")
    if t['kind'] == 4 and t['name'] == 'mm_struct':
        print(f"=== mm_struct tid={t['tid']} size={t['size']} vlen={t['vlen']}")
        for m in t['extra']['members']:
            print(f"    {m[0]:<32} off=0x{m[2]//8:x}")
    if t['kind'] == 4 and t['name'] == 'futex_state':
        print(f"=== futex_state tid={t['tid']} size={t['size']} vlen={t['vlen']}")
        for m in t['extra']['members']:
            print(f"    {m[0]:<32} off=0x{m[2]//8:x} tid={m[1]}")
    if t['kind'] == 4 and t['name'] == 'page':
        print(f"=== page tid={t['tid']} size={t['size']} vlen={t['vlen']}")
        for m in t['extra']['members']:
            print(f"    {m[0]:<32} off=0x{m[2]//8:x}")
    if t['kind'] == 4 and t['name'] == 'slab_cache':
        print(f"=== slab_cache tid={t['tid']} size={t['size']} vlen={t['vlen']}")
        for m in t['extra']['members']:
            print(f"    {m[0]:<32} off=0x{m[2]//8:x}")